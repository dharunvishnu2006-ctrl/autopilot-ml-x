...
6. **`test_concurrent_beats_serial` depends on generated benchmark
   ...
   been run once.
7. **PostgreSQL setup is blocked on Windows.** The note records the unresolved port 5432 permission issue, Administrator attempt, Defender/AV checks, and that Steps 155–156 are deferred while SQLite continues for G1–G7
...
8. Defer Step 169 because PostgreSQL partitioning is unavailable while the G1 PostgreSQL blocker remains unresolved. Same as Steps 155–156     
