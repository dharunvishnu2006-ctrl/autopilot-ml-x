def test_inner_join_returns_all_matched_runs(conn):
    rows = conn.execute(
...
def test_left_join_anti_pattern_finds_unrun_experiments(conn):
    rows = conn.execute(
...
def test_self_join_finds_same_model_pairs(conn):
    rows = conn.execute(
...
def test_self_join_excludes_self_pairs(conn):
    rows = conn.execute(
        "SELECT a.id, b.id FROM runs a "
        "JOIN runs b ON a.model_type_id = b.model_type_id "
        "WHERE a.id < b.id"
